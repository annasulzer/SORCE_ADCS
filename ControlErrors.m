...
for i = 1:length(DCM_out)
    DCM_target(:, :, i) = targetDCM(state_sun_out(i, 1:3)', R_princ);
    DCM_error(:, :, i) = DCM_target(:, :, i) * DCM_out(:, :, i)';
end
...
euler = DCMseries2eulerseries(DCM_out);
target_euler = DCMseries2eulerseries(DCM_target);
...
subplot(3,1,1)
hold on;
plot(t_out, rad2deg(alignment_error(:, 1)- target_euler(:, 1)), 'blue')
xlabel('t [s]')
ylabel('\Delta\phi [deg]')
title('Alignment Error over time (Euler 213 sequence)')
subplot(3,1,2)
hold on;
plot(t_out, rad2deg(alignment_error(:, 2)- target_euler(:, 2)), 'blue')
xlabel('t [s]')
ylabel('\Delta\theta [deg]')
subplot(3,1,3)
hold on;
plot(t_out, rad2deg(alignment_error(:, 3) - target_euler(:, 3)), 'blue')
xlabel('t [s]')
ylabel('\Delta\psi [deg]')
...
% subplot(3,1,1)
% hold on;
% plot(t_out, rad2deg(target_euler(:, 1)), 'blue')
% xlabel('t [s]')
% ylabel('\Delta\phi [deg]')
% title('Alignment Error over time (Euler 213 sequence)')
% subplot(3,1,2)
% hold on;
% plot(t_out, rad2deg(target_euler(:, 2)), 'blue')
% xlabel('t [s]')
% ylabel('\Delta\theta [deg]')
% subplot(3,1,3)
% hold on;
% plot(t_out, rad2deg(target_euler(:, 3)), 'blue')
% xlabel('t [s]')
% ylabel('\Delta\psi [deg]')
